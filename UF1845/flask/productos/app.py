...
def editar(id):
    ...
    if request.method == 'POST':
        ...
        conn.execute('''update producto set codigo=?, nombre=?, precio=?, cantidad=?, fecha_caducidad=? where id=? ''',(codigo,nombre,precio,cantidad,fecha_caducidad,id))
...
def crear():
    if request.method == 'POST':
        ...
        conn.execute('''insert into producto(codigo,nombre,precio,cantidad,fecha_caducidad)
                    values (?,?,?,?,?)''',(codigo,nombre,precio,cantidad,fecha_caducidad))
        conn.commit()
        conn.close()
        return redirect(url_for('index'))
